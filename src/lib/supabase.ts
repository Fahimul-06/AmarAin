...
class QueryBuilder {
  ...
  private async execute(){
    try {
      if(this.mode==='query') return await request(`/data/${this.table}/query`,{method:'POST',body:JSON.stringify({filters:this.filters,order:this.orderBy,limit:this.maxRows,single:this.singleRow,select:this.selectText})});
      if(this.mode==='insert') return await request(`/data/${this.table}/insert`,{method:'POST',body:JSON.stringify({rows:this.payload,returning:this.returnRows})});
      if(this.mode==='update') return await request(`/data/${this.table}/update`,{method:'POST',body:JSON.stringify({filters:this.filters,values:this.payload,returning:this.returnRows})});
      return await request(`/data/${this.table}/delete`,{method:'POST',body:JSON.stringify({filters:this.filters})});
...
async function currentSession(){
  ...
  try { const data=await request('/auth/me'); return {access_token:token(),user:{...data.user,user_metadata:{full_name:data.profile?.full_name,phone:data.profile?.phone,role:data.profile?.role,preferred_language:data.profile?.preferred_language}}}; } catch { localStorage.removeItem(TOKEN_KEY); return null; }
}
